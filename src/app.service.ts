...
export class OrderService implements OnModuleInit {
  ...
  async createOrder(data: { createOrderDto: CreateOrderDto; token: string }) {
    const { createOrderDto, token } = data;
    if (!createOrderDto || !createOrderDto.idempotenceKey) {
      throw new BadRequestException('Missing idempotency key or invalid input');
    }

    const { userId } = await this.validateToken(token);
    // Check for duplicate order (idempotency)
    const existingOrder = await this.orderModel.findOne({
      idempotencyKey: createOrderDto.idempotenceKey,
    });
    if (existingOrder) {
      return {
        message: 'Order already exists',
  ...
  async getOrderStatus(data: { userId: string }) {
    ...
    return {
      ...
      orders: orders.map((order) => ({
        id: order._id,
        userId: order.userId,
        idempotencyKey: order.idempotenceKey,
        status: order.status,
        isCancelled: order.isCancelled,
      })),
    };
  ...
  async updateOrderStatus(data: { userId: string; statusValue: OrderStatus }) {
    ...
    for (const order of orders) {
      ...
      if (order.status === data.statusValue) {
        continue; // Skip orders already in the requested status
      }
      order.status = data.statusValue;
      await order.save();
    }
    ...
    return {
      message: 'Order status updated successfully for eligible orders',
    };
  }
